...
class SAConvLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, att_hidden_dim, kernel_size, bias):
        ...
        # self.attention_layer_h_c = SA_Attn_Mem(hidden_dim, att_hidden_dim)
        # self.attention_layer_c_longc = SA_Attn_Mem(hidden_dim, att_hidden_dim)
        # self.attention_layer_longc_longlongc = SA_Attn_Mem(hidden_dim, att_hidden_dim)
        # self.attention_layer_h_x = SA_Attn_Mem(hidden_dim, att_hidden_dim)

        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=3*self.hidden_dim,
        ...
        # self.conv_long_long = nn.Sequential(
        #     nn.Conv2d(in_channels=self.input_dim + self.hidden_dim,
        #               out_channels=4 * self.hidden_dim,
        #               kernel_size=kernel_size,
        #               padding=self.padding,
    ...
    def forward(self, input_tensor, cur_state):
        ...
        h_cur, c_cur, m_cur ,long_c_cur,pre_h = cur_state
        # h_cur, c_cur = self.attention_layer_h_c(h_cur, c_cur)
        # c_cur,long_c_cur = self.attention_layer_c_longc(c_cur,long_c_cur)
        # long_c_cur,long_long_c_cur = self.attention_layer_c_longc(long_c_cur,long_long_c_cur)
        ...
        combined = torch.cat([input_tensor, h_cur,pre_h], dim=1)
        ...
        # # 2次嵌套，⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇这里是新插入的内容，为了改编LSTM结构，如果没用，最后要删除这部分。
        # combined_long_long = torch.cat([c_next, long_c_next], dim=1)
        # combined_conv_long_long = self.conv_long_long(combined_long_long)
        # long_long_cc_i, long_long_cc_f, long_long_cc_o, long_long_cc_g = torch.split(combined_conv_long_long, self.hidden_dim, dim=1)
        # long_long_i = torch.sigmoid(long_long_cc_i)
        # long_long_f = torch.sigmoid(long_long_cc_f)
        # long_long_o = torch.sigmoid(long_long_cc_o)
        # long_long_g = torch.tanh(long_long_cc_g)
        #
        # long_long_c_next = long_long_f * long_long_c_cur + long_long_i * long_long_g
        # long_c_next = long_long_o * torch.tanh(long_long_c_next)
        # # 2次嵌套，⬆⬆⬆⬆⬆⬆⬆⬆⬆⬆⬆⬆⬆⬆这里是新插入的内容，为了改编LSTM结构，如果没用，最后要删除这部分。
        # #
        ...
        return h_next, c_next, m_next ,long_c_next,h_cur
    ...
    def init_hidden(self, batch_size, image_size,device):
        ...
        long_c = torch.zeros(batch_size, self.hidden_dim, height, width,device=device)
        long_long_c = torch.zeros(batch_size, self.hidden_dim, height, width,device=device)
        return h, c, m, long_c,long_long_c
