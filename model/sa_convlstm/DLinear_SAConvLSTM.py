...
class SAConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        ...
        h_cur, c_cur, m_cur ,res_cur = cur_state
        ...
        h_next, m_next ,res_next = self.attention_layer(h_next, m_cur,res_cur)
        return h_next, c_next, m_next ,res_next
...
class SAConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
            h, c, m ,res = hidden_state[layer_idx]
            output_inner = []
            for t in range(seq_len):
                h, c, m ,res = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, t, :, :, :], cur_state=[h, c, m ,res])
                output_inner.append(h)
            layer_output = torch.stack(output_inner, dim=1)
            cur_layer_input = layer_output
            layer_output_list.append(layer_output)
            last_state_list.append([h, c, m ,res])
