...
class Encode2Decode(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, attn_hidden_dim, kernel_size, img_size=(16, 16), num_layers=4,
        ...
        # encode:降低图片分辨率
        self.img_encode_0 = nn.Sequential(
            nn.Conv2d(input_dim, hidden_dim, (1, 1), (1, 1)),
        )
        self.img_encode_1 = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, (3, 3), (2, 2), (1, 1)),
        )
        self.img_encode_2 = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, (3, 3), (2, 2), (1, 1)),
        )
        self.img_encode_ac_0 = nn.Sequential(
            nn.Conv2d(input_dim, hidden_dim, (1, 1), (1, 1)),
            nn.Sigmoid(),
        )
        self.img_encode_ac_1 = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, (3, 3), (2, 2), (1, 1)),
            nn.Sigmoid(),
        )
        self.img_encode_ac_2 = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, (3, 3), (2, 2), (1, 1)),
            nn.Sigmoid(),
        )


        self.img_decode_0 = nn.Sequential(
            nn.ConvTranspose2d(hidden_dim, hidden_dim, (3, 3), (2, 2), (1, 1), output_padding=(1, 1)),
        )
        self.img_decode_1 = nn.Sequential(
            nn.ConvTranspose2d(hidden_dim, hidden_dim, (3, 3), (2, 2), (1, 1), output_padding=(1, 1)),
        )
        self.img_decode_2 = nn.Sequential(
            nn.Conv2d(hidden_dim, input_dim, (1, 1), (1, 1)),
        )
        self.img_decode_ac_0 = nn.Sequential(
            nn.ConvTranspose2d(hidden_dim, hidden_dim, (3, 3), (2, 2), (1, 1), output_padding=(1, 1)),
            nn.Sigmoid(),
        )
        self.img_decode_ac_1 = nn.Sequential(
            nn.ConvTranspose2d(hidden_dim, hidden_dim, (3, 3), (2, 2), (1, 1), output_padding=(1, 1)),
            nn.Sigmoid(),
        )
        self.img_decode_ac_2 = nn.Sequential(
            nn.Conv2d(hidden_dim, input_dim, (1, 1), (1, 1)),
            nn.Sigmoid(),
        )
    ...
    def forward(self,ite,pre_epoch, x, y,generator,discriminator,optimizer_D,optimizer_G, teacher_forcing_rate=0.5, hidden_state=None,val_flag=False):
        ...
        for t in range(seq_len+horizon-1):
            ...
            x = self.img_encode_0(x) * self.img_encode_ac_0(x)
            x = self.img_encode_1(x) * self.img_encode_ac_1(x)
            x = self.img_encode_2(x) * self.img_encode_ac_2(x)
            ...
            out = self.img_decode_0(out) * self.img_decode_ac_0(out)
            out = self.img_decode_1(out) * self.img_decode_ac_1(out)
            out = self.img_decode_2(out) * self.img_decode_ac_2(out)
